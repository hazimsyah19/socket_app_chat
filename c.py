...
class GUI:
    ...
    def __init__(self):
        ...
        image0 = Image.open("/root/Documents/python/socket_chat_app/161026056787944340.png")
    ...
    def receive(self): 
        while True: 
            try: 
                ...
                temp = decrypt(msg)
                message = temp.decode()
                # SEND CLIENT's NAME if the server ask for ot
                if message == 'NAME':
                ...
                # CLIENT QUIT
                    ...
                    self.textCons.config(state = DISABLED) 
                    self.textCons.see(END,client.close())
                    self.root.destroy()
                else: 
